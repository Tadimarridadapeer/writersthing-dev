...
export default function EditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="h-screen flex flex-col pt-0 bg-white text-zinc-900">
      {/* Editor Header */}
      <div className="h-16 border-b border-zinc-200 bg-white flex items-center justify-between px-6 shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            className="flex items-center gap-2 px-4 py-2 border border-black font-bold text-xs uppercase tracking-widest hover:bg-black hover:text-white transition-all text-black bg-white"
          >
            <Save size={14} />
            Save Draft
          </button>
          <button className="flex items-center gap-2 px-4 py-2 bg-black text-white font-bold text-xs uppercase tracking-widest hover:opacity-80 transition-all shadow-lg">
            <Send size={14} />
            Publish
          </button>
      ...
      <div className="flex-grow flex overflow-hidden">
        ...
        {/* Content Area */}
        <div className="flex-grow overflow-y-auto bg-white px-6 py-12 md:px-24">
          <motion.div 
...
function ToolIcon({ icon, active }: any) {
  return (
    <button className={`p-3 transition-all rounded-sm ${active ? "bg-black text-white shadow-lg" : "text-zinc-400 hover:bg-zinc-100 hover:text-black"}`}>
      {icon}
    </button>
  );
}
...
function OutlineItem({ label, active }: any) {
  return (
    <div className={`p-3 border rounded-sm text-xs font-bold transition-all cursor-pointer ${active ? "border-black bg-zinc-50 text-black" : "border-transparent text-zinc-400 hover:text-zinc-600"}`}>
      {label}
    </div>
  );
}
