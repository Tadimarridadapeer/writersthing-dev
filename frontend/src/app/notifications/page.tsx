  ...
  const getMessage = (type: string, metadata: any) => {
    switch (type) {
      ...
      case "new_like": return metadata?.title ? (
        <span>
          liked <span className="font-bold text-zinc-900">"{metadata.title}"</span>.
        </span>
      ) : "liked your work.";
      ...
      case "new_comment": 
        if (metadata?.text) {
          ...
          return (
            <span>
              commented on {metadata?.title ? <span className="font-bold text-zinc-900">"{metadata.title}"</span> : "your post"}: <span className="italic text-zinc-500">"{truncated}"</span>
            </span>
          );
        }
        return metadata?.title ? (
          <span>
            commented on <span className="font-bold text-zinc-900">"{metadata.title}"</span>.
          </span>
        ) : "commented on your post.";
      case "reply_to_comment": return "replied to your comment.";
      case "book_published": return (
        <span>published a new book: <span className="font-bold text-zinc-900">"{metadata?.title || 'Untitled'}"</span></span>
      );
      case "story_published": return (
        <span>published a new story: <span className="font-bold text-zinc-900">"{metadata?.title || 'Untitled'}"</span></span>
      );
